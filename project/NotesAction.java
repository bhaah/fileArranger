...
public class NotesAction extends Action{
    ...
    private static final String COOSE_COURSE = ">choose a course:";
    ...
    public void run() {
        ...
        while(!shouldFinish){
            ...
            if(actionType!=null) dealWithAction();
                ...
                initActionType(stream.waitForInputAndGet());
            }
        }
        closeActionOnStream();
    }
    ...
    private void initCourseFromInput() {
        stream.addToPrint(COOSE_COURSE);
    ...
    private void removeNote() {
        if(param!=null){
            ...
            stream.addToPrint(UIResponses.adminSay("line removed :)\n"));
        }
    }

    private void printNote() {
        ArrayList notes = courseNotesFile.getSeparetedLines();  
        stream.addToPrint(UIResponses.adminSay("printing all the notes:\n"));
        for(int lineNum = 0 ; lineNum<notes.size();lineNum++){
            stream.addToPrint(UIResponses.color(UIResponses.blue,"["+lineNum+"]"+notes.get(lineNum)+"\n"));
        } 
    }

    private void addNote() {
        courseNotesFile.writeNewLine(param);
        stream.addToPrint(UIResponses.adminSay("adding new line :)\n"));
    }
